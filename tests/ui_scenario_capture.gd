...
func _ready() -> void:
	...
	if not builder.activate(scenario_id):
		...
		get_tree().quit(3)
		return
	if not _reject_retired_industrial_network_arguments():
		get_tree().quit(4)
		return
...
func _reject_retired_industrial_network_arguments() -> bool:
	for argument_value in OS.get_cmdline_user_args():
		var argument := String(argument_value)
		if argument.begins_with("--network-") or argument.begins_with("--industry-section=") or argument.begins_with("--industry-view="):
			push_error("UI_SCENARIO_CAPTURE_ERROR: Industrial Network capture arguments are retired; capture the Factory workspace instead")
			return false
	return true
