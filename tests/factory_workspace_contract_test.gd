...
func _test_versioned_application_intents() -> void:
	...
	var connect_intent := {
		...
		"payload":{"link_kind":"power", "source_id":"power", "target_id":"mine", "item_id":"bogus-item", "ignored_untrusted_field":{"nested":true}}
	}
	...
	_check(not (connected.get("request", {}).get("payload", {}) as Dictionary).has("ignored_untrusted_field"), "durable command receipts keep only canonical command payload fields")
	_check(str(connected.get("request", {}).get("payload", {}).get("item_id", "invalid")) == "", "POWER receipts expose the canonical item-free link contract")
	...
	var revision_conflict_intent := connect_intent.duplicate(true)
	revision_conflict_intent["base_runtime_revision"] = int(connect_intent.get("base_runtime_revision", -1)) + 1
	var revision_conflict: Dictionary = game.execute_factory_command(revision_conflict_intent)
	_check(not bool(revision_conflict.get("accepted", true)) and str(revision_conflict.get("reason_code", "")) == "COMMAND_ID_CONFLICT", "a command id cannot be reused against a different base revision")
	var serialized_connect_state = JSON.parse_string(JSON.stringify(game.state.to_dictionary()))
	game.state = SpaceGameState.from_dictionary(serialized_connect_state as Dictionary, database.domains.keys(), database.regions)
	var persisted_connect_replay: Dictionary = game.execute_factory_command(connect_intent)
	_check(bool(persisted_connect_replay.get("accepted", false)) and bool(persisted_connect_replay.get("replayed", false)) and game.state.factory_worlds["intent-grid"].get("links", {}).size() == 1, "a canonical CONNECT receipt remains idempotent after JSON save/load")
...
func _test_factory_construction_intents() -> void:
	...
	var empty_funding_revision_before := int(storage_world.get("runtime_revision", -1))
	var empty_funding_delivery_before: Dictionary = storage_order.get("delivered_items", {}).duplicate(true)
	...
	_check(not bool(empty_funding.get("accepted", true)) and str(empty_funding.get("reason_code", "")) == "INPUT_SHORTAGE" and empty_funding.get("events", []).is_empty(), "a zero-material funding attempt is rejected without a success event")
	_check(not game.state.factory_worlds["construction-intent-grid"].get("command_receipts", {}).has("contract-fund-empty"), "a rejected zero-material funding attempt does not create a durable receipt")
	var empty_funding_world: Dictionary = game.state.factory_worlds["construction-intent-grid"]
	_check(int(empty_funding_world.get("runtime_revision", -1)) == empty_funding_revision_before and empty_funding_world.get("construction_orders", {}).get(storage_order_id, {}).get("delivered_items", {}) == empty_funding_delivery_before, "rejected zero-material funding leaves the order and runtime revision unchanged")
...
func _test_factory_site_storage_capability() -> void:
	...
	_check(bool(factory.place_entity_immediate(world, "grid_construction_yard", Vector2i(0, 0), "", "capacity-yard").get("ok", false)), "storage projection fixture places a construction yard with a local work buffer")
	_check(bool(factory.place_entity_immediate(world, "grid_fluid_tank", Vector2i(32, 0), "", "capacity-fluid").get("ok", false)), "storage projection fixture places a typed fluid depot")
...
func _test_facility_providers_are_location_bound() -> void:
	var state := SpaceGameState.create_new(database.domains.keys(), database.regions)
	var simulation := SimulationEngine.new(database)
	var remote_world := factory.create_world("remote-provider-grid", "lunar_space", Vector2i(256, 128), 902)
	_check(bool(factory.place_entity_immediate(remote_world, "grid_solar_array", Vector2i(0, 0), "", "remote-power").get("ok", false)), "provider fixture places remote power")
	_check(bool(factory.place_entity_immediate(remote_world, "grid_research_complex", Vector2i(32, 0), "", "remote-research").get("ok", false)), "provider fixture places a remote Research Complex")
	_check(bool(factory.place_entity_immediate(remote_world, "grid_repair_dock", Vector2i(64, 0), "", "remote-repair").get("ok", false)), "provider fixture places a remote Repair Dock")
	_check(bool(factory.place_entity_immediate(remote_world, "grid_starport_expansion_v", Vector2i(96, 0), "", "remote-starport-v").get("ok", false)), "provider fixture places a remote Starport expansion")
	_check(bool(factory.connect_entities(remote_world, "POWER", "remote-power", "remote-research").get("ok", false)), "provider fixture powers remote Research")
	_check(bool(factory.connect_entities(remote_world, "POWER", "remote-power", "remote-repair").get("ok", false)), "provider fixture powers remote repair")
	factory.advance_world(remote_world, 1000.0)
	state.factory_worlds = {"remote-provider-grid":remote_world}
