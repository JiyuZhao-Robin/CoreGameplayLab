...
func _test_fixed_canvas_matrix() -> void:
	var cases := [
		{"window":Vector2(1440, 900), "scale":1.0, "offset":Vector2.ZERO},
		{"window":Vector2(1920, 1080), "scale":1.2, "offset":Vector2(96, 0)},
		{"window":Vector2(2560, 1440), "scale":1.6, "offset":Vector2(128, 0)},
		{"window":Vector2(1366, 768), "scale":768.0 / 900.0, "offset":Vector2((1366.0 - 1440.0 * 768.0 / 900.0) * 0.5, 0)},
		{"window":Vector2(900, 1200), "scale":0.625, "offset":Vector2(0, 318.75)}
	]
	for test_case in cases:
		...
		_check(content_rect.position.is_equal_approx(expected_offset), "%s centers the fixed canvas with the expected letterbox offset" % window_size)
		_check(content_rect.size.is_equal_approx(Policy.DESIGN_VIEWPORT_SIZE * expected_scale), "%s scales the complete 1440x900 canvas as one surface" % window_size)
		...
		_check(is_equal_approx(content_rect.size.aspect(), Policy.DESIGN_VIEWPORT_SIZE.aspect()), "%s preserves the 16:10 design aspect without distortion" % window_size)


func _test_window_independent_theme_scale() -> void:
	for window_size in [Vector2(1366, 768), Vector2(1440, 900), Vector2(1920, 1080), Vector2(2560, 1440)]:
	...
	_check(Policy.logical_usable_size(Vector2(1, 1), 2.0).is_equal_approx(Policy.DESIGN_VIEWPORT_SIZE), "logical usable size is always the 1440x900 design canvas")


func _test_fixed_layout_profile() -> void:
	for logical_size in [Vector2(640, 360), Vector2(1440, 900), Vector2(3840, 2160)]:
