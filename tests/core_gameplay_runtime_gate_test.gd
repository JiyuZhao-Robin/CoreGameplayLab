...
func _complete_deep_system(packet: Dictionary, _outer_battleship_id: String) -> void:
	...
	var titan_actual_shortfalls := {}
	for item_value in titan_expected_costs:
		var item_id := str(item_value)
		var target := int(titan_expected_costs.get(item_id, 0))
		var shortfall := maxi(0, target - _earth_location_and_storage_quantity(item_id))
		if shortfall > 0:
			titan_actual_shortfalls[item_id] = shortfall
	var titan_steel_shortfall := int(titan_actual_shortfalls.get("steel_composite", 0))
	var titan_titanium_shortfall := int(titan_actual_shortfalls.get("titanium_alloy", 0))
	_check(titan_steel_shortfall == 10 and titan_titanium_shortfall == 2 and not titan_actual_shortfalls.has("superalloy") and not titan_actual_shortfalls.has("antimatter_cell") and not titan_actual_shortfalls.has("quantum_component"), "the deterministic Deep program preserves its high-tier bootstrap stock and exposes the exact regional Titan shortfalls; actual=%s" % JSON.stringify(titan_actual_shortfalls))
	if failures.size() > 0:
		return
	_prepare_earth_steel_shortfall(int(titan_expected_costs.get("steel_composite", 0)), packet, "J10 canonical Deep Survey Titan steel closure")
	if failures.size() > 0:
		return
	_return_lunar_titanium_to_earth(titan_titanium_shortfall, packet, "J10 canonical Deep Survey Titan titanium closure")
	if failures.size() > 0:
		return
...
func _move_asteroid_cobalt_ore_to_jovian(quantity: int, packet: Dictionary, label: String) -> void:
	...
	var asteroid_extractor_id := str(packet.get("asteroid_cobalt_extractor_id", ""))
	var jovian_world_id := str(packet.get("jovian_world_id", ""))
	var jovian_storage_id := str(packet.get("jovian_storage_id", ""))
	for link_value in _snapshot(asteroid_world_id).get("links", []):
		var link := link_value as Dictionary
		if str(link.get("kind", "")) == "POWER" and str(link.get("target_id", "")) == asteroid_extractor_id:
			var removed := _factory_command("REMOVE_LINK", {"link_id":str(link.get("id", ""))}, asteroid_world_id)
			_check(bool(removed.get("accepted", false)), "%s freezes its Asteroid cobalt extractor before bounded freight" % label)
	_clear_competing_cargo_outputs(asteroid_extractor_id, "cobalt_ore", "", asteroid_world_id)
	if failures.size() > 0:
		return
	var chunk_count := ceili(float(quantity) / 8.0)
	for chunk_index in range(chunk_count):
		var chunk := mini(8, quantity - chunk_index * 8)
		var asteroid_storage := _entity(_snapshot(asteroid_world_id), asteroid_storage_id)
		var stored := int((asteroid_storage.get("inventory", {}) as Dictionary).get("cobalt_ore", 0))
		if stored < chunk:
			_extract_resource_batch(asteroid_extractor_id, "cobalt_ore", str(packet.get("asteroid_power_id", "")), asteroid_storage_id, chunk - stored, "%s extraction %d/%d" % [label, chunk_index + 1, chunk_count], asteroid_world_id)
		if failures.size() > 0:
			return
		...
		if failures.size() > 0:
			return
		var stored_before_export := int((_entity(_snapshot(asteroid_world_id), asteroid_storage_id).get("inventory", {}) as Dictionary).get("cobalt_ore", 0))
		_check(stored_before_export >= chunk, "%s retains the exact Asteroid cobalt source cargo after operating-reserve delivery; required=%d stored=%d" % [label, chunk, stored_before_export])
		if failures.size() > 0:
			return
...
## Count only custody that can be staged through public Factory/Location
## commands.  Machine inputs and outputs remain bound to their recipes.
func _earth_location_and_storage_quantity(item_id: String) -> int:
	var earth_snapshot := _snapshot(EARTH_WORLD_ID)
	var total := int((earth_snapshot.get("location_available_inventory", {}) as Dictionary).get(item_id, 0))
	for entity_value in earth_snapshot.get("entities", []):
		var entity := entity_value as Dictionary
...
## Close only the steel that is absent from Earth staging custody.  Existing
## regional inputs are first normalized into the primary Factory store so the
## local recipe never mistakes Location stock for directly consumable input.
func _prepare_earth_steel_shortfall(target_quantity: int, packet: Dictionary, label: String) -> void:
	var steel_shortfall := maxi(0, target_quantity - _earth_location_and_storage_quantity("steel_composite"))
	if steel_shortfall <= 0:
		return
	var external_requirements := {}
	_accumulate_external_requirements("steel_composite", steel_shortfall, external_requirements)
	if failures.size() > 0:
		return
	var storage_id := str(packet.get("storage_id", ""))
	for item_value in external_requirements:
		var item_id := str(item_value)
		var required := int(external_requirements.get(item_id, 0))
		_consolidate_factory_item(item_id, required, storage_id, label)
		if failures.size() > 0:
			return
		var earth_snapshot := _snapshot(EARTH_WORLD_ID)
		var stored := int((_entity(earth_snapshot, storage_id).get("inventory", {}) as Dictionary).get(item_id, 0))
		var available_at_location := int((earth_snapshot.get("location_available_inventory", {}) as Dictionary).get(item_id, 0))
		var import_quantity := mini(maxi(0, required - stored), available_at_location)
		if import_quantity > 0:
			_import_from_location(item_id, import_quantity, storage_id, "%s existing regional input custody" % label)
		if failures.size() > 0:
			return
	_prepare_external_for_manifest({"steel_composite":steel_shortfall}, packet, label)
