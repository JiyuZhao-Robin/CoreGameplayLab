...
func _run() -> void:
	...
	add_child(main)
	await _redraw()
	_check(_has_text_fragment(main, "恒星系") and _has_text_fragment(main, "库存") and _has_text_fragment(main, "诊断"), "Chinese fixed core navigation is rendered")
	...
	await _redraw()
	_check(main.find_child("FactoryWorkspace", true, false) != null and main.find_child("FactoryCanvas", true, false) != null, "Chinese Industry navigation mounts the real Factory workspace")
	_check(_has_text_fragment(main, I18n.t("factory.palette.construction")) and I18n.t("factory.palette.construction") != "Construction palette", "Chinese Factory controls use the zh-CN catalog")
	_press(main.find_child("Navigation_ships", true, false) as Button)
	await _redraw()
	_press(main.find_child("FleetSection_readiness", true, false) as Button)
	await _redraw()
	var empty_fleet_resupply := main.find_child("AutoResupplyFleet", true, false) as Button
	_check(empty_fleet_resupply != null and empty_fleet_resupply.disabled, "empty Expedition Fleet resupply remains visibly disabled")
	_check(empty_fleet_resupply != null and empty_fleet_resupply.tooltip_text == I18n.t("notice.expedition_fleet_empty"), "empty Expedition Fleet reason uses the zh-CN UI catalog")
