...
func _run() -> void:
	...
	var location_selector := main.find_child("LocationSelector", true, false) as OptionButton
	_check(_select_option_metadata(location_selector, "earth_orbit"), "Industry sidebar Location selector exposes Earth")
	await _settle()
	_check(
		str(main.get("_selected_location_id")) == "earth_orbit"
		and str(main.get("_selected_factory_world_id")) == "earth-surface-grid"
		and _selected_world(main) == "earth-surface-grid",
		"Industry Location changes keep the sidebar, selected Factory world, and rendered snapshot synchronized"
	)
	world_selector = main.find_child("FactoryWorldSelector", true, false) as OptionButton
	_check(_select_option_metadata(world_selector, lunar_world_ids[0]), "Factory selector can return to the Lunar workspace after a Location-driven switch")
	await _settle()
