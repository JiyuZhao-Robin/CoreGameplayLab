...
func _test_placement_and_port_contracts() -> void:
	...
	_check(str(factory.connect_entities(world, "POWER", "power", "mine", "bogus-item").get("reason_code", "")) == "DUPLICATE_LINK" and world.get("links", {}).size() == 2, "POWER duplicate detection canonicalizes ignored item payload before writing topology")
