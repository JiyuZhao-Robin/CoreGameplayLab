...
func _factory_definition_available(definition: Dictionary, candidate_state: SpaceGameState = null) -> bool:
	...
	for requirement_value in definition.get("requirements", []):
		if not simulation.requirement_met(evaluated_state, requirement_value as Dictionary):
			return false
	return true
...
func execute_factory_command(intent: Dictionary) -> Dictionary:
	...
	var request_fingerprint := _factory_command_request_fingerprint(request_record)
	if not state.factory_worlds.has(world_id):
	...
	if previous_receipt_value is Dictionary:
		...
		var previous_request_value = previous_receipt.get("request", null)
		var request_matches := false
		if previous_request_value is Dictionary:
			request_matches = _factory_requests_match(previous_request_value as Dictionary, request_record)
		else:
		...
		previous_receipt["message"] = _factory_command_success_message(command_kind)
	...
	match command_kind:
		...
		"SET_RECIPE":
			...
			if not content.factory_recipes.has(configured_recipe_id) or not _factory_definition_available(content.factory_recipes[configured_recipe_id], transaction.working_state):
				operation_result = {"ok":false, "reason_code":"RECIPE_LOCKED", "reason":I18n.t("factory.reason.recipe_locked", "This Factory recipe is still locked.")}
			else:
	...
	last_notice = _factory_command_success_message(command_kind)
...
func _factory_command_success_message(command_kind: String) -> String:
	match command_kind:
		...
		"FUND_CONSTRUCTION_FROM_LOCATION":
			return I18n.t("factory.success.fund_construction_from_location")
		"SET_RECIPE":
		...
		"EXPORT_TO_LOCATION":
			return I18n.t("factory.success.export_to_location")
	...
	match command_kind:
		...
		"FUND_CONSTRUCTION":
			if typeof(raw_payload.get("order_id", "")) not in text_types or typeof(raw_payload.get("storage_id", "")) not in text_types:
				return {"ok":false}
			return {"ok":true, "payload":{"order_id":str(raw_payload.get("order_id", "")), "storage_id":str(raw_payload.get("storage_id", ""))}}
		"FUND_CONSTRUCTION_FROM_LOCATION":
		...
		"IMPORT_FROM_LOCATION", "EXPORT_TO_LOCATION":
			if typeof(raw_payload.get("storage_id", "")) not in text_types \
					...
					or typeof(raw_payload.get("quantity", 0)) not in number_types:
				return {"ok":false}
			return {"ok":true, "payload":{
...
func _factory_command_request_fingerprint(request: Dictionary) -> String:
	return JSON.stringify(_factory_canonical_value(request))


func _legacy_factory_command_request_fingerprint(protocol_version: int, command_kind: String, world_id: String, payload: Dictionary) -> String:
...
func research_runtime_snapshot() -> Dictionary:
	...
	return {
		...
		"stage_consumed":(runtime.get("stage_consumed", {}) as Dictionary).duplicate(true),
		"reserved_costs":(runtime.get("reserved_costs", {}) as Dictionary).duplicate(true)
	}
...
func _new_transaction() -> GameStateTransaction:
	var transaction := GameStateTransaction.new(state, content.domains.keys())
	# from_dictionary deliberately strips all disposable projections. Rebuild the
	# working state from authoritative physical/domain state before any command
	# performs transaction-local validation or mutation.
	simulation.ensure_frontier_state(transaction.working_state)
	return transaction


func _commit_transaction(transaction: GameStateTransaction) -> void:
	# Rebuild every disposable runtime view from authoritative Survey markers,
	# Megastructure projects and physical Factory worlds before publishing state.
	# This protects non-Factory commands from committing a partial projection.
	simulation.ensure_frontier_state(transaction.working_state)
