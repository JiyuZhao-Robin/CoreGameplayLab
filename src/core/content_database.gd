...
func _validate_factory_grid_content() -> void:
	...
	for region_id_value in regions.keys():
		...
		if width > 0 and height > 0:
			...
			for resource_region_value in resource_regions.values():
				var resource_region := resource_region_value as Dictionary
				if str(resource_region.get("region", "")) != location_id:
					continue
				for resource_id_value in resource_region.get("resources", []):
					var resource_id := str(resource_id_value)
					if items.has(resource_id) and not generated_resource_ids.has(resource_id):
						generated_resource_ids.append(resource_id)
			generated_resource_ids.sort()
			if not generated_resource_ids.is_empty():
				var final_resource_index := generated_resource_ids.size() - 1
				var final_resource_end := Vector2i(32 + (final_resource_index % 4) * 64 + 24, 32 + (final_resource_index / 4) * 64 + 24)
				if final_resource_end.x > width or final_resource_end.y > height:
					errors.append("factory world profile '%s' cannot contain its generated resource fields" % location_id)
	...
	var starter_entity_ids := {}
	var starter_entity_rects: Array[Rect2i] = []
	for entity_value in starter.get("entities", []):
		...
		if entity_id.is_empty() or not factory_buildings.has(definition_id):
			errors.append("factory starter entity has invalid identity or building definition")
		elif starter_entity_ids.has(entity_id):
			errors.append("factory starter entity has duplicate identity '%s'" % entity_id)
		else:
			starter_entity_ids[entity_id] = true
			var entity_origin_value: Variant = entity.get("origin", {})
			var entity_origin: Dictionary = entity_origin_value as Dictionary if entity_origin_value is Dictionary else {}
			var definition: Dictionary = factory_buildings.get(definition_id, {})
			var footprint: Dictionary = definition.get("footprint", {})
			var entity_rect := Rect2i(
				int(entity_origin.get("x", -1)),
				int(entity_origin.get("y", -1)),
				int(footprint.get("width", 0)),
				int(footprint.get("height", 0))
			)
			if entity_rect.position.x < 0 or entity_rect.position.y < 0 or entity_rect.size.x <= 0 or entity_rect.size.y <= 0 or entity_rect.end.x > int(starter_size.get("x", 0)) or entity_rect.end.y > int(starter_size.get("y", 0)):
				errors.append("factory starter entity '%s' exceeds starter bounds" % entity_id)
			else:
				for occupied_rect in starter_entity_rects:
					if entity_rect.intersects(occupied_rect):
						errors.append("factory starter entity '%s' overlaps another starter entity" % entity_id)
						break
				starter_entity_rects.append(entity_rect)
