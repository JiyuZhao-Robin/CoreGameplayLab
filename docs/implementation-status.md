...
| Save Migration | 部分实现 | schema 24→39 显式迁移；35→36 只创建空 `factory_worlds`，不会凭空猜测历史设施坐标；37→38 剥离旧舰船工作插件、迁移物流方舟并初始化有限残骸状态；38→39 为旧存档中已调查的远端地点补记有限调查前哨包。旧资产进入画布的可视迁移向导尚待实现。 |
| 本地化 | 部分实现 | zh-CN / en 的当前 Factory 命令、状态、实体和交互键已对齐，并由目录审计及中英文 smoke 覆盖；历史 UI 命名空间仍有待逐步清理的 parity debt。 |
