...
## 2026-09-11 无人机物流替代道路

当前本地物流权威为[无人机物流设计](Planetary_Drone_Logistics_zh_CN.md)。已接入无人机塔、圆形覆盖、10 件取货、取货时缺料优先、十批原料目标和返塔共享存储；旧道路数据保留为不运行的历史归档。正式供电不再依赖道路。新的运输、内容、无道路生产链、存档与 UI 检查结果集中记录在该文档，不沿用下文道路时代的测试结果。
...
2026-09-11 最新用户决定：**暂时只保留完全平地与现有矿脉**，暂停山脉、水域、森林、自然对象采集、山体开挖及挖掘机选型。正式地形查询、显示、检查器与建造统一返回平地，新游戏与旧存档均适用。原生成输入和差量记录保留；仅矿区生成继续使用历史地理筛选，以保持矿脉位置、类型、品位和种子。详见[地表状态](./Earth_Terrain_zh_CN.md)。

此前 Kenney Nature Kit 原包、72 个树/树桩/木材 GLB、CC0 许可与图集保留，但当前不加载或绘制树木。此前地貌和[巨型挖掘机调查](./Giant_Excavator_Candidates_zh_CN.md)均为暂停记录，不是当前实施任务。
...
- **外观已认可并接入**：制造工厂接入自动制造机及 DSP 制造台三等级，燃料精炼厂接入原油精炼厂，化工处理站接入化工厂/量子化工厂，热能工厂接入火力发电厂。热能底座从居中 5×5 预览改为校准的 5×6 比例，等比放入正式 10×10 占地；其余设备保留 12×10。实体、图标、物品、幽灵、摆放和可见图片点选共用美术及锚点。见[候选建筑说明](./Factory_Building_Candidates_zh_CN.md)。
- **已验证**：主代理实际运行新 `factory_approved_industry_test.gd`，检查四类真实 4K 动画、熄灯停帧、三档缩放底座包围、图标缓存、原始导入 mipmap、幽灵与预览、减少动态效果/过期快照/屏幕外停帧及状态隔离；热能使用真实独立煤炭/道路负荷及燃料耗尽夹具，其余三类运行速率使用展示快照。另通过电弧炉、矿机、DSP 美术、物品美术和工厂图层回归。全部原始素材与正式派生帧在项目内，四套来源/派生哈希检查通过。此轮没有满负载性能基准结论。
