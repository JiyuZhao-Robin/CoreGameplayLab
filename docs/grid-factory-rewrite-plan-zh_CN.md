...
> 历史范围：本文保留 2026-09-01、Schema 36 初次方格工厂重构的方案和术语，用于理解迁移原因。它**不是当前待办或验收规范**；下文所有“当前”“已实现”“P1/P2/P3”和 CARGO/POWER/Port Transfer/现场施工表述都只指当时基线。
>
> 当前 Factory 规则、实现状态和待办分别见[道路工业设计](./Planetary_Industry_Roads_Design_zh_CN.md)、[实现状态](./implementation-status.md)和[当前剩余工作](./remaining-work.md)。当前普通工业使用道路、Location 单库存和成品部署；不恢复端口拖线、实体仓储复制、现场 BOM/工时、预放建筑或太阳能开局。
>
> 本文保留真实舰船、研究、迁移背景和早期确定性世界设计的历史记录，但不得据此新增旧玩法、测试或 UI。
